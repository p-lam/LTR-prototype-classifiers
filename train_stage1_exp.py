...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, cls_num_list, resampled_dataloader=None, block=None):
    ...
    # end = time.time()
    for i, (real_data, resampled_data) in enumerate(zip(train_loader, resampled_dataloader)):
        images, target = real_data
        sample_feat, sample_target = resampled_data
        if i > end_steps:
            break
...
# 
        if torch.cuda.is_available():
            ...
            sample_feat = sample_feat.cuda(config.gpu, non_blocking=True)
            sample_target = sample_target.cuda(config.gpu, non_blocking=True)
        ...
        logits_synthetic = classifier(sample_feat)
        softmax = torch.nn.Softmax(dim=-1).cuda()
        logsoftmax = torch.nn.LogSoftmax(dim=-1).cuda()
        entropy_loss = lambda logits: (-1 * (softmax(logits)*logsoftmax(logits)).mean())
        loss_synthetic = entropy_loss(logits_synthetic)
