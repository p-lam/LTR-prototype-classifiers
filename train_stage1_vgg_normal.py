...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    plot_loss(train_losses, val_losses, config.num_epochs, config)

def plot_loss(train_loss, test_loss, epochs, config):
    """
    Helper function to plot loss curves
    """
    loss_dir = "./loss_curves2"
    ...
    plt.savefig(loss_dir + "/" + str(config.name))
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, dataset, block=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if config.mixup is True:
            mixed, targets_a, targets_b, _, x1, x2 = mixup_data(images, target, alpha=config.alpha)
            if config.dataset == 'places':
                with torch.no_grad():
                    feat_a = model(images)
            ...
            loss = mixup_criterion(criterion, output, targets_a, targets_b, lam, output_mixed, y, target)
        else:
            if config.dataset == 'places':
                with torch.no_grad():
                    feat_a = model(images)
